...
public class DisplayImp implements Display {
  ...
  * @param playerBoard the label of the board
  */

  @Override
  public void displayBoard(BattleBoard[][] grid, String playerBoard) {
    displayString(playerBoard);
    appendable = new StringBuilder();
    try {
      for (int i = 0; i < grid.length; i++) {
        for (int j = 0; j < grid[i].length; j++) {
          appendable.append(grid[i][j] + " ");
        }
        appendable.append("\n");
      }
